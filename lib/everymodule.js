      ...
      if (arg.constructor === Object) {
        ...
        if (!this[property])
          this[property] = function (setTo) {
            var k = '_' + property;
            if (!arguments.length) {
              // TODO this.readabilityauth is not yet available here in some contexts
              //      For example, when we set and try to access a scope in an auth module definition
              //      but if you look in index, readabilityauth is not assigned to the module until after it is
              //      required
              if (this.readabilityauth && this.readabilityauth.debug && 'undefined' === typeof this[k]) {
